...
/// - 4 bytes: (two byte scan code) + (two byte scan code)
///
...
/// - 4 bytes: (2-byte make code) + (2-byte make code)
///
...
/// - 6 bytes: (3-byte break code) + (3-byte break code)
///
/// Exceptions:
...
// A representation of a key that allows us to map clearly keysyms to scan codes.
#[derive(Debug)]
...
impl TryFrom<KeyEvent> for KeyEventRep {
    ...
    fn try_from(keyevent: KeyEvent) -> Result<Self, Self::Error> {
        let (base_val_1, base_val_2) = match keyevent.keysym {
            ...
            // XXX: ignoring pause key for now
            _ => (0x0, 0x0),
        };
