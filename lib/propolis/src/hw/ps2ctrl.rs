...
/// Virtual PS/2 Controller (Intel 8042)
///
///
///
/// TODO: block comment
...
/// - control port
///
/// commands: discuss how only a byte comes in at a time
///
/// registers:
/// - control status
///
/// How mouse and keyboard data arrive
///
/// interrupts
///
/// picture with vnc, controller, keyboard, mouse
...
mod probes {
    ...
    fn ps2ctrl_keyevent_lost(keysym_val: u32, scan_code_set: u8) {}
}

// TODO: write docs
bitflags! {
    /// Status Register
    /// This register contains flags indicating the status of the PS/2 controller.
    /// It is accessed by the OS via the Control I/O port.
    #[derive(Default)]
    pub struct CtrlStatus: u8 {
        ...
        /// Input Buffer Status
        /// This bit must be cleared to 0 before the OS attempts to write data to the data port or
        /// the control port.
        const IN_FULL = 1 << 1;
        ...
        /// Command/Data
        /// This bit is 0 if data written to the input buffer is for a device; 1 indicates the data
        ...
        /// Parity Error (0 = no error, 1 = timeout error)
        const PARITY = 1 << 7;
    }
}
...
bitflags! {
    ...
    /// This is the output format for the Control I/O port.
    #[derive(Default)]
...
impl PS2Ctrl {
    ...
    pub fn keyevent(&self, ke: KeyEvent) {
        ...
        match KeyEventRep::try_from(ke) {
            ...
            Err(_) => {
                // drop any unrecognized keys
                // XXX: should we log when this happens?
                probes::ps2ctrl_keyevent_lost!(|| {
    ...
    fn pio_rw(&self, port: u16, rwo: RWOp) {
        ...
        match port {
            ...
            // Control Port
            // Reads are from the Status Register
            // Writes go to the Control Output Port
...
// XXX: is there a reason this needs to be 16?
